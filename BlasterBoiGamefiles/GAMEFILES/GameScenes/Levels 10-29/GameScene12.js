class GameScene12 extends Phaser.Scene {
    constructor(){
        super({key:"GameScene12"})
    }
    preload(){
      ...
      create(){
        ...
        let win = false;
        const goal = 600; // unit - score
        const rateOfFire = 300; // unit - ms
        const genEn = 130; // unit - ms
        const rarity = 210; // chance of special items
        const leveln = 13; // level after this one
        const damage = 2; // damage enemys do
        const currentL = 12; // current level
        const regenDelay = 1910; // regen rate
        const bounusH = 10; // health gained by geting health pack
