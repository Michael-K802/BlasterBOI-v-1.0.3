class GameScene15 extends Phaser.Scene {
    constructor(){
        super({key:'GameScene15'})
    }
    preload(){
      ...
      create(){
        ...
        let win = false;
        const goal = 750; // unit - score
        const rateOfFire = 300; // unit - ms
        const genEn = 150; // unit - ms
        const rarity = 200; // chance of special items
        const leveln = 16; // level after this one
        const damage = 6; // damage enemys do
        const currentL = 15; // current level
