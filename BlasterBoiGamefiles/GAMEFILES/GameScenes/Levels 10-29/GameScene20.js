class GameScene20 extends Phaser.Scene {
    constructor(){
        super({key:"GameScene20"})
    }
    preload(){
      ...
      this.load.image('AI','GAMEFILES/Assets/PlayerStuff/space-ship(revamp)-player-pixilart.png');
      }
      create(){
        ...
        let win = false;
        const goal = 1000; // unit - score
        const rateOfFire = 300; // unit - ms
        const genEn = 130; // unit - ms
        const rarity = 6; // chance of special items 
        const damage = 1; // damage enemys do
        const currentL = 20; // current level
        const regenDelay = 2010; // regen rate
        const bounusH = 20; // health gained by geting health pack
        const scoreGet = .5; // amount of points awarded apon collecting coin
        ...
        gameState.regenMin = 30;
        gameState.maxAmmo = 1;
        // player and stats 
        gameState.player = this.physics.add.sprite(150,500,'player')
        gameState.player.setCollideWorldBounds(true);
        gameState.health = 100;
        gameState.score = 0;
        gameState.ammo = 0.5;
        gameState.scoreText = this.add.text(10,570,`${unit}:${gameState.score}/${goal}`,{fontSize:'20px',fill:'#f6faff'});
        gameState.healthText = this.add.text(10,545,'Health:100',{fontSize:'20px',fill:'#f6faff'});
        ...
        // AI player and stats 
        gameState.AI = this.physics.add.sprite(450,500,'AI');
        console.log(typeof gameState.AI.x)
        gameState.AIHealth = 100;
        gameState.AIhealthText = this.add.text(10,515,'Escort Health: 100',{fontSize:'20px',fill:'#f6faff'})
        gameState.AI.setCollideWorldBounds(true);
        // shooting and stuff
        ...
        this.physics.add.overlap(healthPack,shot,(healthP)=>{
          healthP.destroy();
          if (gameState.AIHealth >= 131 && gameState.AIHealth <= 149 ){
            let overflow = gameState.AIHealth + bounusH -150;
            gameState.AIHealth += bounusH;
            gameState.AIHealth -= overflow;
            gameState.AIhealthText.setText(`Escort Health:${gameState.AIHealth}`);
            let bp = overflow * 2;
            gameState.score += bp;
            gameState.scoreText.setText(`${unit}:${gameState.score}/${goal}`)
          } else if (gameState.AIHealth <= 130){
            gameState.AIHealth += bounusH;
            gameState.AIhealthText.setText(`Escort Health:${gameState.AIHealth}`)
          } else if (gameState.AIHealth === 150){
            gameState.score += 40;
            gameState.scoreText.setText(`${unit}:${gameState.score}/${goal}`)
        ...
        this.physics.add.overlap(coins,shot,(coin)=>{
          ...
          if (gameState.score >= goal){
            ...
            this.add.text(150,350,`Click to continue`,{fill:'#22ff13',fontSize:'25px' })
            gameState.gameOver = true
            this.input.on('pointerup',()=>{
              this.scene.stop(`GameScene20`);
      ...
      this.physics.add.overlap(enemys,gameState.player,(enemy)=>{
        ...
        function AIM(){
          ...
          if (gameState.AI.x > 500){
              gameState.right = false;
              gameState.left = true;
          } else if (gameState.AI.x < 100){
              gameState.right = true;
              gameState.left = false;
          } else if (gameState.AI.y < 150){
            gameState.up = false;
            gameState.down = true;
          }else {
            if (randomNum === 0){
        ...
        function AIshoot(){
            shot.create(gameState.AI.x,gameState.AI.y,'shot');
            shot.setVelocityY(-1010);
          }
        ...
        this.physics.add.overlap(enemys,gameState.AI,()=>{
            if( win === true){

            } else if (!dT){

            } else if (gameState.AIHealth > 0){
                gameState.AIHealth -= damage;
                gameState.AIhealthText.setText(`Escort Health: ${gameState.AIHealth}`);
            } else if (gameState.AIHealth < 1){
                gameState.reGen.destroy()
                this.physics.pause();
                this.add.text(250,300,'Game Over',{fill:'#ff1000',fontSize:'25px' })
                this.add.text(200,350,'Click to restart',{fill:'#ff1000',fontSize:'25px' })
      ...
      update(){
        ...
        if (gameState.up && !gameState.gameOver){
            gameState.AI.y -= gameState.upSpeed;
          } else {
            gameState.AI.setVelocityY(0);
          }
          if (gameState.down && !gameState.gameOver){
            gameState.AI.y += gameState.downSpeed;
          } else {
            gameState.AI.setVelocityY(0)
          }
          if (gameState.left && !gameState.gameOver){
            gameState.AI.x -= gameState.sideSpeed;
          } else {
            gameState.AI.setVelocityY(0);
          }
          if (gameState.right && !gameState.gameOver){
            gameState.AI.x += gameState.sideSpeed;
          } else {
            gameState.AI.setVelocityY(0)
          }
