class GameScene13 extends Phaser.Scene {
    constructor(){
        super({key:'GameScene13'})
    }
    preload(){
      ...
      create(){
        ...
        let win = false;
        const goal = 650; // unit - score
        const rateOfFire = 300; // unit - ms
        const genEn = 130; // unit - ms
        const rarity = 210; // chance of special items
        const leveln = 14; // level after this one
        const damage = 1; // damage enemys do
        const currentL = 13; // current level
        const regenDelay = 1910; // regen rate
        ...
        gameState.healthText = this.add.text(10,545,'Health:100',{fontSize:'20px',fill:'#f356f6'});
      ...
      this.physics.add.overlap(coins,gameState.player,(enemy)=>{
        if (win === true){
          // so you don't die when you win
        } else if (!dT){
          // invincability
