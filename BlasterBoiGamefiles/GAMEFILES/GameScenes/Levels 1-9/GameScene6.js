class GameScene6 extends Phaser.Scene {
    ...
    create(){
      ...
      let win = false;
      const goal = 250; // unit - score
      const rateOfFire = 500; // unit - ms
      const genEn = 180; // unit - ms
      const rarity = 400; // chance of medpack
      const leveln = 7; // level after this one
      const damage = 1; // damage enemys do
      const currentL = 6; // current level
      const regenDelay = 1299; // regen rate
      const bounusH = 25; // health gained by geting health pack
      ...
      gameState.upSpeed = 4 // up
      gameState.downSpeed = 3 // down
      gameState.maxHealth = 95;
      gameState.regenMin = 50;
      gameState.maxAmmo = 1;
      // player and stats 
      ...
      gameState.score = 0;
      gameState.ammo = 0;
      this.add.text(10,570,'Reload Status: REDUCED',{fontSize:'20px',fill:'#f356f6'})
      gameState.scoreText = this.add.text(10,520,`${unit}:${gameState.score}/${goal}`,{fontSize:'20px',fill:'#fdf1da'});
      ...
      function coinGen(){
        let xCoord = Math.random()*600;
        coins.create(xCoord,-70,'coins').setScale(.5);
      }
      const coinR = this.time.addEvent({
          delay: 1500,
          callback: coinGen,
          callbackScope: this,
          loop: true,
