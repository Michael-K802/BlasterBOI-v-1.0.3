class GameScene3 extends Phaser.Scene {
    ...
    create(){
      ...
      const rarity = 400000000000000000000000; // chance of medpack
      const leveln = 4; // level after this one
      const damage = 1; // damage enemys do
      const currentL = 3; // current level
      ...
      function coinCreate(enemy){
        ...
        let y2 = enemy.y -= rN3
        if (rN1 === 0){
          healthPack.create(enemy.x,enemy.y,'healthPack').setScale(.75);
        } else {
            if (rN2 === 0){
